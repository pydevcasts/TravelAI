    ...
    <div class="relative z-20 h-35 md:h-65">
      ...
      <div class="absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
        <label for="cover"
          ...
          <input id="cover" class="sr-only" name="cover" type="file" />
          <span>
            <svg class="fill-current" fill="none" height="14" viewBox="0 0 14 14" width="14"
    ...
    <div class="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
      <div
        ...
        <div class="relative drop-shadow-2">
          <img v-if="userStore.user.avatar" :src="userStore.user.avatar" alt="Profile Image"
            class="rounded-full h-full w-full object-cover" />
          <img v-else src="/src/assets/images/user/user-06.png" alt="Default Profile Image"
            class="rounded-full h-full w-full object-cover" />
          <label
            ...
            <input id="profile" class="sr-only" name="profile" type="file" />
          </label>
        </div>
      </div>
      <div v-if="userStore.user" class="mt-4">
        <h3 class="mb-1.5 text-2xl font-medium text-black dark:text-white"> {{ userStore.user.email }} </h3>
        ...
        <div class="mx-auto max-w-180">
          ...
          <p class="mt-4.5 text-sm font-normal">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque posuere fermentum urna, eu condimentum
            mauris tempus ut. Donec fermentum blandit aliquet. Etiam dictum dapibus ultricies. Sed vel aliquet libero.
...
const user = ref(null); // Create a reactive reference for user data

const fetchUserDetails = async () => {
  const userId = userStore.user?.pk; // Use optional chaining

  if (!userId) return; // Exit if userId is not available

  try {
    const response = await instance.get(`/api/users/${userId}/`);
    userStore.setUser(response.data); // Set user data in the store
    user.value = response.data; // Also update local user reference
  } catch (error) {
    console.error('Error fetching user details:', error);
    userStore.clearUser(); // Clear user on error if necessary
  }
};

onMounted(() => {
  if (userStore.user) {
    console.log("User store check...", userStore.user);
    user.value = userStore.user; // Use user data from store if available
  } else {
    fetchUserDetails(); // Fetch user details from API
  }
});
</script>
